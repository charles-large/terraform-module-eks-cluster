resource "aws_iam_role" "aws_alb_controller" {
  name = "${var.eks_cluster_name}-aws_alb_controller_sa_role"
  description = "IAM Role for AWS ALB Controller in EKS Pod"
...
resource "aws_iam_role_policy" "aws_alb_controller" {
  name = "${var.eks_cluster_name}-aws_alb_controller_policy"
  role = aws_iam_role.aws_alb_controller.name
  policy = jsonencode(
    {
    "Version": "2012-10-17",
    "Statement": [
